...
public class Computador4 extends JFrame implements ActionListener {
	JButton arreglo[] = new JButton[64];//arreglo para los botones
	static Carta cartas[] = new Carta[64];//arreglo para las cartas
	...
	public Computador4(String nombreJug) {
	...
	public void Jugar() {
		for (int i = 0; i < arreglo.length; i++) {
			i = (int) (Math.random() * 16); //el numero aleatorio es dependiendo la dificultad 
			if (cartas[i].revelado == false) {// seleccionar dos cartas
				...
				if (par == 0) {
					...
					if (cartas[i].valor == temporal.valor) {
						...
						JOptionPane.showMessageDialog(panel, "Turno del jugador: " + nombre1);
						try {
							// actualizar boton
			...
			if (cartas[i].boton == e.getSource() && cartas[i].revelado == false) {// seleccionar dos cartas
				...
				if (par == 0) {
					...
					if (cartas[i].valor == temporal.valor) {
						...
						JPanel panel= new JPanel();
						panel.setBackground(new Color(255, 255, 255));
						JOptionPane.showMessageDialog(panel, "Turno del jugador: " + nombre2);
						try {
							// actualizar boton
							cartas[i].boton.update(cartas[i].boton.getGraphics());
							Thread.sleep(500);
	...
	public static void main(String nombreJug) {
		new Computador4(nombreJug).setVisible(true);
